...
import { updateCarAdmin, getCarAdmin, getCarImageUrl, listBrands, Brand } from "@/lib/api-admin"
import {
  Select,
  SelectContent,
  SelectItem,
...
export default function EditVehiclePage() {
  ...
  async function loadCar() {
    try {
      ...
      setImagesToKeep(carImages) // Inicialmente mantém todas
    } catch (err: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await updateCarAdmin(carId, {
        ...
        imagesToKeep: imagesToKeep, // Lista de imagens existentes a manter
      })
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto">
      ...
      <div className="flex items-center gap-4 mb-4">
        <Link href={`/admin/inventory/${carId}`} className="text-muted-foreground hover:text-primary transition-colors flex items-center gap-1 text-sm font-medium">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-card p-6 rounded-xl shadow-sm border border-border">
          ...
          <div className="space-y-6">
            ...
            {/* Images Grid */}
            {(imagesToKeep.length > 0 || images.length > 0) && (
              <div>
                <p className="text-sm font-medium text-foreground mb-4">
                  {imagesToKeep.length + images.length} {imagesToKeep.length + images.length === 1 ? 'imagem' : 'imagens'}
                </p>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                  {/* Existing Images */}
                  {existingImages.map((imageUrl, index) => {
                    const isKept = imagesToKeep.includes(imageUrl)
